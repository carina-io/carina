...
openssl req -new -key "${tmpdir}"/tls.key -subj "/CN=${service}.${namespace}.svc" -days 36500 -out "${tmpdir}"/tls.csr -config "${tmpdir}"/csr.conf
...
spec:
  ...
  request: $(< "${tmpdir}"/tls.csr base64 | tr -d '\n')
  usages:
  - digital signature
  - key encipherment
  - server auth
