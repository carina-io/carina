...
func (lv2 *lvm2Implement) CreateSnapshot(snap, lv, vg string) (string, error) {
	// Pool容量时lv卷的三倍，则能创建两个快照
	// TODO: 需要检查pool>lvm卷,若是相等则不支持创建快照操作
	return lv2.Executor.ExecuteCommandWithOutput("lvcreate", "-s", fmt.Sprintf("%s/%s", vg, lv), "-n", snap, "-ay", "-Ky")
}
