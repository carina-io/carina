...
const (
	...
	ephVolConKey     = "csi.storage.io/ephemeral"
)

// NewNodeService returns a new NodeServer.
func NewNodeService(nodeName string, conn *grpc.ClientConn, service *LogicalVolumeService) csi.NodeServer {
	return &nodeService{
		nodeName:     nodeName,
...
type nodeService struct {
	...
	k8sLVService *LogicalVolumeService
	mu           sync.Mutex
}
...
func (s *nodeService) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if isInlineEphemeralVolumeReq {
		...
		if lv == nil {
			...
			lv, err = s.getLvFromContext(ctx, utils.DefaultDeviceClassName, volumeID)
			if err != nil {
				return nil, err
			}
		}
	...
	if err != nil {
		if isInlineEphemeralVolumeReq {
			...
			if _, err = s.lvService.RemoveLV(ctx, &proto.RemoveLVRequest{Name: volumeID, DeviceClass: utils.DefaultDeviceClassName}); err != nil {
...
func (s *nodeService) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	if info.IsDir() {
		...
		volume, err := s.getLvFromContext(ctx, utils.DefaultDeviceClassName, volID)
		if err != nil {
			return nil, err
		}
...
func (s *nodeService) NodeExpandVolume(ctx context.Context, req *csi.NodeExpandVolumeRequest) (*csi.NodeExpandVolumeResponse, error) {
	...
	deviceClass := utils.DefaultDeviceClassName
	if err == nil {
